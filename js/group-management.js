...
function createGroupCard(group) {
  ...
  const domainName = standardData
    ? getDomainDisplayName(standardData.domain)
    : getDomainDisplayName(group.domain);
  const clusterName = standardData
    ? standardData.cluster || standardData.name
    : group.cluster || "No standard selected";
  const description = standardData
    ? getStandardDescription(standardData)
    : 'Click "Edit Group" to choose a math standard';
  const domainIcon = standardData
    ? getDomainIcon(standardData.domain)
  ...
  const sampleQuestion = standardData
    ? generateSampleQuestionForCard(standardData)
    : getDefaultSampleQuestion();
            ...
            <div class="top-content">
                <div class="domain-cluster-standard-container">
                    <div class="header-container">
                        ...
                        <div class="card-domain-title">Grade ${grade} » ${domainName}</div>
                    </div>
                ...
                <div class="example-question-container">
                    <div class="example-question-content">
                        ...
                        <div class="example-question-text">
                            <span class="question-type-italic">${
                              sampleQuestion.type
                            }: </span>${sampleQuestion.text}
                        </div>
                    </div>
                    <div class="view-all-questions-button" onclick="openExampleQuestionsModal('${
                      group.standard
                    }')">
            ...
            <div class="bottom-content">
                <div class="session-length-container">
                    ...
                    <div class="session-length-segmented-control" 
                        ...
                        <button class="session-length-option ${
                          sessionLength.min === 10 && sessionLength.max === 15
                            ? "active"
                            : ""
                        }"
                                role="radio"
                                aria-checked="${
                                  sessionLength.min === 10 &&
                                  sessionLength.max === 15
                                    ? "true"
                                    : "false"
                                }"
                                tabindex="${
                                ...
                                aria-describedby="session-length-help-${
                                  group.id
                                }"
                                data-min="10" 
                                data-max="15"
                                onclick="selectSessionLength(${
                                  group.id
                                }, 10, 15, this)">
                            10-15 Min
                        </button>
                        <button class="session-length-option ${
                          sessionLength.min === 15 && sessionLength.max === 20
                            ? "active"
                            : ""
                        }"
                                role="radio"
                                aria-checked="${
                                  sessionLength.min === 15 &&
                                  sessionLength.max === 20
                                    ? "true"
                                    : "false"
                                }"
                                tabindex="${
                        ...
                        <button class="session-length-option ${
                          sessionLength.min === 20 && sessionLength.max === 25
                            ? "active"
                            : ""
                        }"
                                role="radio"
                                aria-checked="${
                                  sessionLength.min === 20 &&
                                  sessionLength.max === 25
                                    ? "true"
                                    : "false"
                                }"
                                tabindex="${
                ...
                <div class="cta-actions-container">
                    <button class="edit-group-button" onclick="editGroup(${
                      group.id
                    })">
...
function updateGroup(groupId, updates) {
  ...
  if (updates.standard) {
    ...
    groupSlots[groupIndex].url = generateStudentUrl(
      groupSlots[groupIndex].studentCode
    );
...
function selectSessionLength(groupId, minTime, maxTime, buttonElement) {
  ...
  const segmentedControl = buttonElement.closest(
    ".session-length-segmented-control"
  );
  const allOptions = segmentedControl.querySelectorAll(
    ".session-length-option"
  );
...
function handleSegmentedControlKeydown(event) {
  if (
    !["ArrowLeft", "ArrowRight", "Home", "End", "Space", "Enter"].includes(
      event.key
    )
  ) {
    return;
  ...
  const segmentedControl = currentButton.closest(
    ".session-length-segmented-control"
  );
  const allOptions = Array.from(
    segmentedControl.querySelectorAll(".session-length-option")
  );
...
function updateModalHeader(standardData) {
  ...
  const gradeText = document.querySelector(
    ".example-questions-header .grade-text"
  );
  const domainText = document.querySelector(
    ".example-questions-header .domain-text"
  );
  const domainIcon = document.querySelector(".domain-icon-svg");

  if (gradeText) gradeText.textContent = `Grade ${standardData.grade}`;
  if (domainText)
    domainText.textContent = getDomainDisplayName(standardData.domain);
  if (domainIcon) {
  ...
  if (clusterTitle)
    clusterTitle.textContent = standardData.cluster || standardData.name;
...
function createQuestionElement(question, index) {
  ...
  const originalTitle =
    ...
    question.title.replace(/Problem \d+: /, "");
...
function nextQuestion() {
  const nextIndex =
    (currentQuestionIndex + 1) % currentStandardQuestions.length;

  // Add fade out/in animation
  if (window.gsap) {
    const currentQuestion = document.querySelector(".carousel-question.active");
    const nextQuestionEl =
      document.querySelectorAll(".carousel-question")[nextIndex];
...
function previousQuestion() {
  ...
  if (window.gsap) {
    const currentQuestion = document.querySelector(".carousel-question.active");
    const prevQuestionEl =
      document.querySelectorAll(".carousel-question")[prevIndex];
